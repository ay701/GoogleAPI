...
//					curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
		...
		//	public function DownloadCSV($site, $savepath=".")
			public function DownloadCSV($site, $savepath="GWT_report")
			{
				if(self::IsLoggedIn() === true) {
					...
					foreach($tables as $table) {
						...
						echo("<span style='margin-top:15px; margin-bottom:15px; font-size:16px; width:250px;' id='$table' class='$table' onClick='showHideDiv($table)'>+&nbsp; $table &nbsp;&nbsp;&nbsp;</span>");
						...
						elseif($table=="SOCIAL_ACTIVITY") {
							...
							self::writeReport($finalName,$table);
						}
		 ...
		 */
			public function DownloadCSV_XTRA($site, $savepath=".", $tokenUri, $tokenDelimiter, $filenamePrefix, $dlUri)
			{
				if(self::IsLoggedIn() === true) {
					...
					//echo("_finaleName----".$finalName);
					
					//echo("<div style='margin-bottom:10px; cursor:pointer'><img src='/img/down_arrow_blue.png' width=15>&nbsp; $filenamePrefix</div>");
					self::writeReport($finalName,$filenamePrefix);
				} else { return false; }
			}
					...
					if($separated) {
						foreach($this->_errTablesSort as $sortid => $sortname) {
							foreach($this->_errTablesType as $typeid => $typename) {
								...
								self::SaveData($_url,$finalName);
								self::writeReport($finalName,"CRAWL_ERRORS");
							}
						}
					}
					else {
		 ...
		 */
			...
			/**
			* added by Aizizi Yigaimu 02/13/2013
			* output report content, $fname is the full file path
			*/
			private function writeReport($fname,$table)
			{
				echo("<a href='".substr($fname,0,strlen($fname)-3)."pdf'><img src='/img/pdf.png' width='20px'></a><br>");
				echo("<div id='Div_$table' class='Div_$table' style='display:none'>");
				
				if (!file_exists($fname)) {
    				echo "No report for this item at this time.";
				}else{
					if($file = fopen($fname, "r")){
						...
						while(!feof($file))
						{
							...
							foreach($arr as &$val){
								echo("<td style='font-size:14px'>$val</td>");
							}
